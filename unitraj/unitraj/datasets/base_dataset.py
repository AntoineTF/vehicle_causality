...
class BaseDataset(Dataset):
    ...
    def process_data_chunk(self, worker_index):
        ...
        with open(file_path, 'w') as f:
            json.dump(tracks_serializable, f, indent=4)

    def preprocess(self, scenario):
        """
        Preprocess a simulation scenario for use in trajectory prediction and causal analysis.

        This function extracts information from the provided `scenario` dictionary and structures it into a format suitable 
        for training and evaluation. If `return_embeddings` is set to True, it also handles counterfactual data for causal 
        analysis. The function processes:

        - **Track Information:** Extracts agent trajectories, types, and states.
        - **Counterfactual Data:** If `return_embeddings` is True, counterfactual trajectories are processed similarly.
        - **Map Information:** Extracts map features like lanes, road lines, stop signs, and crosswalks.
        - **Traffic Lights:** Processes dynamic map states and traffic light information.

        Parameters:
        - `scenario` (dict): The input dictionary containing scenario data including agent trajectories, map features, 
                            and optional counterfactual data.

        Returns:
        - dict: A dictionary containing processed data with keys for tracks, map information, dynamic map information, 
                and optional counterfactual data if `return_embeddings` is True.

        Raises:
        - AssertionError: If the number of frames processed does not match the expected `total_steps`.
        """
        ...
        # **Counterfactual Handling Only if return_embeddings is True**
        if return_embeddings:
            ...
            cf_tracks = scenario["tracks_info_cf"]
            ...
            for k, v in cf_tracks.items():
                ...
                all_cf_state = [state['position'], state['length'], state['width'], state['height'], state['heading'],
                            state['velocity'], state['valid']]
                all_cf_state = np.concatenate(all_cf_state, axis = -1)
                
                if all_cf_state.shape[0] < ending_fame:
                    all_cf_state = np.pad(all_cf_state, ((ending_fame - all_cf_state.shape[0], 0), (0, 0)))
                all_cf_state = all_cf_state[starting_fame:ending_fame]
                
                assert all_cf_state.shape[0] == total_steps, f'Error: {all_cf_state.shape[0]} != {total_steps}'
                
                cf_track_infos['object_id'].append(k)
                cf_track_infos['object_type'].append(object_type[v['type']])
                cf_track_infos['trajs'].append(all_cf_state)

            # Convert to NumPy array and apply frequency mask
            cf_track_infos['trajs'] = np.stack(cf_track_infos['trajs'], axis=0)
            cf_track_infos['trajs'][..., -1] *= frequency_mask[np.newaxis]
                
                
        # Standard track processing continues
        ...
        # Return value handling for `return_embeddings`
        if return_embeddings:
            ret = {
                'track_infos': track_infos,
                'dynamic_map_infos': dynamic_map_infos,
                'map_infos': map_infos,
                'cf_track_infos': cf_track_infos,
                'nb_cf': len(cf_tracks) 
            }
        ...
        # Handling of the track prediction selection
        if self.config['only_train_on_ego']:
            ...
            if return_embeddings:
                tracks_to_predict_cf = {
                    'track_index': cf_track_infos["object_id"],
                    'difficulty': np.zeros(np.shape(cf_track_infos['object_id']), dtype=int).tolist(),
                    'object_type':[MetaDriveType.VEHICLE] * len(cf_track_infos['object_id'])
                }
                ret["tracks_to_predict_cf"] = tracks_to_predict_cf
            ...
            num_tracks_to_predict = len(tracks_to_predict['track_index'])
        else:
    ...
    def process(self, internal_format):
        """
        Process a simulation scenario into a structured format for trajectory prediction and causal analysis.

        This function processes both factual and (optionally) counterfactual trajectories. If `return_embeddings` 
        is True, additional counterfactual data is included for causal metrics. Key processing steps include:

        - **Track and Object Information:** Extract agent trajectories, types, and masks.
        - **Counterfactual Handling:** If `return_embeddings` is True, counterfactual data is processed similarly.
        - **Map Information:** Process map polylines and mask unused features.
        - **Data Transformation:** Convert all numerical data to float32 for consistency.

        Parameters:
        - `internal_format` (dict): A pre-structured dataset containing scenario data such as agent trajectories, 
                                    map features, and optional counterfactual trajectories.

        Returns:
        - list: A list of dictionaries where each entry contains structured data for a single object in the scenario.

        Raises:
        - AssertionError: If data shapes or lengths are inconsistent.
        - Warning: If map data is missing.
        """
        ...
        # Counterfactual Data Handling
        if return_embeddings: 
            track_index_to_predict_cf = np.array(info['tracks_to_predict_cf']['track_index'])
            cf_track_infos = info["cf_track_infos"]
            cf_trajs = cf_track_infos["trajs"]
            cf_obj_trajs_past = cf_trajs[:, :current_time_index + 1]
            cf_obj_trajs_future = cf_trajs[:, current_time_index + 1:]
            center_objects_cf = cf_track_infos["trajs"][:, current_time_index, :]
            obj_type_cf = np.array(cf_track_infos['object_type'])
            sample_num_cf = center_objects_cf.shape[0]
        ...
        # Generate structured data for factual and counterfactual (if applicable)
        if return_embeddings and info["dataset"] == "metadrive":
            ...
            center_gt_trajs_mask, center_gt_final_valid_idx, track_index_to_predict_new, obj_trajs_data_cf, obj_trajs_mask_cf ,obj_trajs_future_state_cf, 
            obj_trajs_future_mask_cf, track_index_to_predict_cf_new ) = self.get_agent_data(
                ...
                obj_types=obj_types,
                center_objects_cf= center_objects_cf, 
                cf_obj_trajs_past = cf_obj_trajs_past, 
                cf_obj_trajs_future = cf_obj_trajs_future, 
                cf_trajs = cf_trajs,
                return_embeddings = return_embeddings, 
                track_index_to_predict_cf = track_index_to_predict_cf, 
                obj_type_cf = obj_type_cf
            )
        else:
        ...
        # Adding counterfactual data to the return dictionary
        if return_embeddings :
            ret_dict.update({
                "obj_trajs_cf": obj_trajs_data_cf,
                "obj_trajs_mask_cf" : obj_trajs_mask_cf,
                'obj_trajs_future_state_cf': obj_trajs_future_state_cf,
                'obj_trajs_future_mask_cf': obj_trajs_future_mask_cf,
                'track_index_to_predict_cf': track_index_to_predict_cf_new,
                'map_center_cf': ret_dict['map_center'].repeat(sample_num_cf, axis=0),
                'dataset_name_cf': [info['dataset']] * sample_num_cf,
                'nb_cf': [info["nb_cf"]]*sample_num_cf
            })
    ...
    def get_agent_data(
            ...
            obj_types, cf_trajs = None ,center_objects_cf = None, return_embeddings = False, cf_obj_trajs_past = None, 
            cf_obj_trajs_future = None,track_index_to_predict_cf = None, obj_type_cf = None):


        # Transform obj_trajs to center coordinates
        num_center_objects = center_objects.shape[0]
        num_objects, num_timestamps, box_dim = obj_trajs_past.shape
        
        # Transform factual trajectories
        ...
        # Initialize counterfactual data structures
        if return_embeddings:
            num_center_objects_cf = center_objects_cf.shape[0]
            num_uniq_scenario, num_timestamps_cf, box_dim_cf = cf_obj_trajs_past.shape
            num_objects_cf = 1 # Only care about the trajectory of the ego car for the counter factuals
            
            obj_trajs_cf = self.transform_trajs_to_center_coords_batch(
            obj_trajs=cf_obj_trajs_past, 
            center_xyz=center_objects_cf[:, 0:3],
            center_heading=center_objects_cf[:, 6],
            heading_index=6, rot_vel_index=[7, 8]
            )
            
            # Reshape to reflect "one counterfactual per batch"
            obj_trajs_cf = obj_trajs_cf.reshape(num_center_objects_cf, 1, num_timestamps_cf, box_dim_cf)
            
            # Prepare counterfactual masks and embeddings
            object_onehot_mask_cf = np.zeros((num_center_objects_cf, num_objects_cf, num_timestamps_cf, 5))
            object_onehot_mask_cf[:, 0, :, 0] = 1 # Vehicle indicator
            object_onehot_mask_cf[:, 0, :, 1] = 0
            object_onehot_mask_cf[:, 0, :, 2] = 0
            object_onehot_mask_cf[:, 0, :, 3] = 1  # Ego vehicle marker
            object_onehot_mask_cf[:, 0, :, 4] = 1


            object_time_embedding_cf = np.zeros((num_center_objects_cf, num_objects_cf, num_timestamps_cf, num_timestamps_cf + 1))
            for i in range(num_timestamps_cf):
                object_time_embedding_cf[:, :, i, i] = 1
            object_time_embedding_cf[:, :, :, -1] = timestamps
            
            object_heading_embedding_cf = np.zeros((num_center_objects_cf, num_objects_cf, num_timestamps_cf, 2))
            object_heading_embedding_cf[:, :, :, 0] = np.sin(obj_trajs_cf[:, :, :, 6])
            object_heading_embedding_cf[:, :, :, 1] = np.cos(obj_trajs_cf[:, :, :, 6])
            
            # Calculate velocity and acceleration
            vel_cf = obj_trajs_cf[:, :, :, 7:9]
            vel_pre_cf = np.roll(vel_cf, shift=1, axis=2)
            acce_cf = (vel_cf - vel_pre_cf) / 0.1
            acce_cf[:, :, 0, :] = acce_cf[:, :, 1, :]
            
            # Combine all features
            obj_trajs_data_cf = np.concatenate([
                obj_trajs_cf[:, :, :, 0:6],
                object_onehot_mask_cf,
                object_time_embedding_cf,
                object_heading_embedding_cf,
                obj_trajs_cf[:, :, :, 7:9],
                acce_cf,
            ], axis=-1)
            
            obj_trajs_mask_cf = obj_trajs_cf[:, :, :, -1]
            obj_trajs_data_cf[obj_trajs_mask_cf == 0] = 0
            
        # Prepare factual masks and embeddings
        ...
        if return_embeddings:
            ...
            _ , num_timestamps_ft_cf, _ = cf_obj_trajs_future.shape
                
            obj_trajs_future_cf = self.transform_trajs_to_center_coords_batch(
                obj_trajs=cf_obj_trajs_future.astype(np.float32),
                center_xyz=center_objects_cf[:, 0:3],
                center_heading=center_objects_cf[:, 6],
                heading_index=6, rot_vel_index=[7, 8]
            )
            
            obj_trajs_future_cf = obj_trajs_future_cf.reshape(num_center_objects_cf, 1, num_timestamps_ft_cf, box_dim_cf)

            obj_trajs_future_state_cf = obj_trajs_future_cf[:, :, :, [0, 1, 7, 8]]  # (x, y, vx, vy)
            obj_trajs_future_mask_cf = obj_trajs_future_cf[:, :, :, -1]
            obj_trajs_future_state_cf[obj_trajs_future_mask_cf == 0] = 0    
                    
            center_obj_idxs = np.arange(len(track_index_to_predict_cf))
            center_gt_trajs_cf = obj_trajs_future_state_cf[center_obj_idxs, 0]
            center_gt_trajs_mask_cf = obj_trajs_future_mask_cf[center_obj_idxs, 0]
            center_gt_trajs_cf[center_gt_trajs_mask_cf == 0] = 0
            
            max_num_agents = self.config['max_num_agents']
            
            # Padding the first dimension instead of the second dimension
            obj_trajs_data_cf = np.pad(
                obj_trajs_data_cf, 
                ((0, max_num_agents + 1 - obj_trajs_data_cf.shape[0]), (0, 0), (0, 0), (0, 0))
            )

            obj_trajs_mask_cf = np.pad(
                obj_trajs_mask_cf, 
                ((0, max_num_agents + 1 - obj_trajs_mask_cf.shape[0]), (0, 0), (0, 0))
            )

            obj_trajs_future_state_cf = np.pad(
                obj_trajs_future_state_cf,
                ((0, max_num_agents + 1 - obj_trajs_future_state_cf.shape[0]), (0, 0), (0, 0), (0, 0))
            )

            obj_trajs_future_mask_cf = np.pad(
                obj_trajs_future_mask_cf,
                ((0, max_num_agents + 1 - obj_trajs_future_mask_cf.shape[0]), (0, 0), (0, 0))
            )
           
            track_index_to_predict_cf_new = np.zeros(len(track_index_to_predict_cf), dtype=np.int64)
            
            obj_trajs_data_cf = np.expand_dims(obj_trajs_data_cf, axis=0)  
            obj_trajs_mask_cf = np.expand_dims(obj_trajs_mask_cf, axis=0)  
            obj_trajs_future_state_cf = np.expand_dims(obj_trajs_future_state_cf, axis=0)
            obj_trajs_future_mask_cf = np.expand_dims(obj_trajs_future_mask_cf, axis=0)
        ...
        # Return results based on whether counterfactual embeddings were included
        if return_embeddings:
             return (obj_trajs_data, obj_trajs_mask.astype(bool), obj_trajs_pos, obj_trajs_last_pos,
                    ...
                    center_gt_final_valid_idx,track_index_to_predict_new, obj_trajs_data_cf, obj_trajs_mask_cf ,
                    obj_trajs_future_state_cf, obj_trajs_future_mask_cf, track_index_to_predict_cf_new)
        else: 
            return (obj_trajs_data, obj_trajs_mask.astype(bool), obj_trajs_pos, obj_trajs_last_pos,
                    obj_trajs_future_state, obj_trajs_future_mask, center_gt_trajs, center_gt_trajs_mask,
    ...
    def transform_trajs_to_center_coords_batch(self, obj_trajs, center_xyz, center_heading, heading_index, rot_vel_index=None):
        """
        Transform trajectories to center coordinates for a batch of simulations, treating each independently.
        Args:
            obj_trajs: (num_scenarios, num_timestamps, num_attrs) - Batch of scenarios.
            center_xyz: (num_scenarios, 3) - Center coordinates for each scenario.
            center_heading: (num_scenarios) - Heading for each scenario.
        Returns:
            obj_trajs_transformed: (num_scenarios, num_timestamps, num_attrs)
        """
        num_scenarios, num_timestamps, num_attrs = obj_trajs.shape
        obj_trajs_transformed = np.zeros_like(obj_trajs)

        # Loop over each scenario to apply the transformation independently
        for i in range(num_scenarios):
            obj_trajs_transformed[i] = self.transform_trajs_to_center_coords(
                obj_trajs[i:i+1],  # Select a single simulation
                center_xyz[i:i+1],
                center_heading[i:i+1],
                heading_index,
                rot_vel_index
            )[0]  # Remove the extra batch dimension

        return obj_trajs_transformed
